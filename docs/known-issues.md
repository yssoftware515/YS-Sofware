...
| D-03 | `ys-web/lib/platform` ~7000 lines dead code | ⚠️ PARTIALLY CORRECTED — see D-03c below | Kernel/registries/contracts/services (1,411 lines) genuinely wired; 865 lines test-only; **4,845 lines across 39 subdirectories unreachable from app code** |
...
**D-03c — Correction (2026-08-21):** The original D-03 verdict ("FALSE POSITIVE") was incorrect. It verified only that the 4 wired subdirectories (kernel, registries, contracts, services) are imported by app code — which was never in dispute — and concluded the entire finding was false. Re-investigation from zero: 100 files, 7,121 lines, 49 subdirectories. **4 subdirectories (1,411 lines)** are wired into app code. **6 subdirectories (865 lines)** are imported only from `tests/platform/`. **39 subdirectories (4,845 lines)** have zero imports from outside `lib/platform/` — they are only cross-referenced by other unreachable platform subdirectories. The ~4,845 unreachable lines are consistent with `docs/frontend.md` §9's description of "standalone abstractions with no production integration — foundation code kept by owner decision," though §9 understates the count (names 8 subsystems; actual count is 39). See Section 4 below for the product-owner recommendation on what to do with this code.
